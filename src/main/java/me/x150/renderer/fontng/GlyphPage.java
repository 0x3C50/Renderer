...
class GlyphPage implements AutoCloseable {
	...
	public record Glyph(int id, int bitmapWidth, int bitmapHeight, int drawOffsetX, int drawOffsetY, AtomicInteger x, AtomicInteger y, GlyphMetrics metrics) {}
	Glyph[] glyphs;
	...
	private void layout(int rowWidth) {
		int w = 0, pX = 0, pY = 0;
		Iterator<Glyph> iterator = Arrays.stream(glyphs)
				.sorted(Comparator.comparingInt(it -> it.bitmapHeight))
				.iterator();
		int rowHeight = 0;
	...
	private void fill() {
		...
		tex = new NativeImageBackedTexture(ni);
		...
		for (Glyph glyph : glyphs) {
			...
			for (int y = 0; y < height; y++) {
				...
				MemoryUtil.memCopy(
						MemoryUtil.memAddress(buffer) + (long) y * width,
						ptr + rowOffset + glyph.x.get(),
						width);
			}
		}
		tex.upload();
